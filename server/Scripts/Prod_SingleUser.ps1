...
$errorLogPath = "\\$logMachine\C$\Users\newhirescript\Documents\Scripts\User\CreateUser\Logs\Error_SingleUser.txt"
$successLogPath = "\\$logMachine\C$\Users\newhirescript\Documents\Scripts\User\CreateUser\Logs\Success_SingleUser.txt"
$exportPath = "\\$logMachine\C$\Users\newhirescript\Documents\Scripts\User\CreateUser\Exports"
$csvPath = "\\$logMachine\C$\Users\newhirescript\Documents\Scripts\User\CreateUser\Exports\CreatedUser.csv"
$newUser = @()
$flag = "success"
$logDirectory2 = "\\$logMachine\C$\Users\newhirescript\Documents\Scripts\User\CreateUser\Logs\BlahBlah.txt"
#$imgPath = "E:\inetpub\wwwroot\WebSite\CORP-NewUserScript\build\public\assets\it_notifications.png"
	...
	try{
		...
		Invoke-Command -ComputerName $logMachine -Credential $credential -ScriptBlock {
			...
			if(Test-Path $logFolder){
				$usrObj | Format-List | Out-File -FilePath $logDirectory -Append 
		...
		} -ArgumentList $logFolder, $logDirectory, $usrObj		
	}
	catch{
		Write-Host "An Error occurred Logging the user" $logMsg
	}
}
...
function ExportUser
{
  ...
  (
	...
	try {
		...
		Invoke-Command -ComputerName $logMachine -Credential $credential -ScriptBlock {
			 ...
			 if(Test-Path $exportPath){
				$usr | Export-Csv $csvPath -NoTypeInformation -Append
		...
		} -ArgumentList $usr, $csvPath, $exportPath
	...
	LogUsr $usr $flag $msg
	Write-Host "Error Issue exporting the user"
	exit
  }
}
...
try{
	...
	$newUser = SetUser $firstName $lastName $startDate $title $legalEntity $state $supervisor $department $email $computer $gender $accountStatus
